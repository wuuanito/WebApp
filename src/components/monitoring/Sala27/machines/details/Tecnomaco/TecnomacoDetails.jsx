...
const TecnomacoDetails = ({ isOpen, onClose, machineData }) => {
  const [activeTab, setActiveTab] = useState('produccion');

  if (!isOpen) return null;

  // Datos de ejemplo para la máquina Tecnomaco
  const tecnomacoData = {
    estado: 'PRODUCIENDO',
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'produccion':
        return <ProduccionTab tecnomacoData={tecnomacoData} />;
      case 'parametros':
        return <OrdenesFabricacionTab />;
      case 'insercion':
        return <InsercionDatosTab tecnomacoData={tecnomacoData} />;
      case 'estadisticas':
        return <EstadisticasTab tecnomacoData={tecnomacoData} />;
      case 'mantenimiento':
        return <MantenimientoTab tecnomacoData={tecnomacoData} />;
      default:
        return <ProduccionTab tecnomacoData={tecnomacoData} />;
    }
  };
      ...
      <div className="bg-white w-full h-full flex flex-col">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Tecnomaco</h1>
            </div>
          </div>
          <button
            onClick={onClose}
...
export default TecnomacoDetails;